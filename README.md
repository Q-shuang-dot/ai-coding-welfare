...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/%E6%95%B0%E6%8D%AE%E6%9B%B4%E6%96%B0-2026--09--22%2006.06%20UTC-informational" alt="数据更新">
</p>
...
> 「首日可得」= 注册基础额度 + 本页邀请链接额度 + 当天能领的签到额度（每日重置额度池的站点按一天的池子算）；模型、价格、在线状态由脚本抓取站点公开接口自动生成，最后更新：`2026-09-22 06:06 UTC`。
>
...
- ⚠ 接口已连续 172 小时没抓到新数据，下列信息为 `2026-09-15 01:48 UTC` 的快照
- 站点名称：**Agent Router**
...
<a href="https://free.supxh.xin/register?code=MVU8GX" target="_blank" rel="noopener"><img src="https://img.shields.io/badge/%E7%AB%8B%E5%8D%B3%E6%B3%A8%E5%86%8C-%E8%82%96%E6%81%A9AI-brightgreen?style=for-the-badge" alt="注册 肖恩AI"></a>
...
- 仅支持 QQ 邮箱注册，需要邮箱验证码
- 邀请码选填，但从本页链接进入并填写 MVU8GX 可额外得 5000 额度
- 注册页有人机校验，遇到拦截属于正常防护
...
client = OpenAI(api_key="你的 Key", base_url="https://free.supxh.xin/v1")
resp = client.chat.completions.create(model="<登录后台查看可用模型名>", messages=[{"role": "user", "content": "ping"}])
print(resp.choices[0].message.content)
```

通用客户端只需填两项：**Base URL** = `https://free.supxh.xin/v1`，**API Key** = 站点后台创建的 Key。
...
- 每日签到可领额度（登录后台操作）
- 邀请他人注册，邀请双方各得 5000 额度
- 额度类型为积分，可用于兑换模型调用

**⚠️ 使用前必读**

- 积分点数和人民币/美元折算未在公开接口暴露，本页按站点公示登记，入账以后台为准
- 模型列表虽可公开读取，但免费/付费线路、模型上下架、倍率调整均可能随时变化
- 该站为新收录站点，稳定性与可用性尚待长期观察
...
- `2026-04-29` gpt-5.5 已上新，支持工具调用，可用于编程开发和自动化工作流。
- `2026-04-10` **可萌中转站开放签到**。本服务为通用大模型 API 接口服务，适用于知识问答、文本创作、编程开发与自动化工作流等场景。使用本服务需合法合规。
- `2026-04-03` 邀请好友注册，双方都会获得 15 个硬币，可用于大模型调用体验。
...
- 从本页邀请链接注册额外：**600 积分**
- 首日合计：**2600 积分**　（本页邀请 600 积分 + 实名认证 2000 积分）
...
- ℹ️ 本次自动探测被站点 WAF 拦下（GitHub Actions 机房 IP 常见，家宽访问不受影响），状态与下列信息沿用 `2026-09-21 21:58 UTC` 的成功快照
- 站点名称：**SeekAi**
- 面板版本：`v1.0.0-rc.25`
- 每日签到：✅
- 开放注册：✅
...
> New API 公益网关 · 系统名「新疆幻城网安科技公益大模型安全网关」，签到 + 任务 + 邀请都能拿额度，注册即送 30000 站内$，免费分组里有标价 0 元一次的模型
...
- 注册就送 30000 站内$（面板 price=7.3，约 ¥219k 面值，本页按站内单位显示、不并入美元合计）
...
CI 每 6 小时抓一次各站接口，与上一次快照逐字段比对，目前已攒下 167 个样本、覆盖约 31.6 天。额度调整、掉线与恢复、模型上下线、价格变动都会自动记一条：
